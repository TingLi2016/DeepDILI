...
train_index = pd.read_csv('/account/tli/CDER/data/fps_extra/train_index_1009.csv')
...
###external dataset
#external = pd.read_csv('/account/tli/CDER/data/external_dataset/external_240_mol2vec.csv')
#cols = data.columns[7:] 
...
base_path = '/account/tli/CDER/results/check/xgboost/xgboost_' + var
...
for col in train_index.columns[9:].values:
    ...
    ### define column name
    col_name = 'xgboost_'+'seed_'+col+'_paras_'+var
    col_name2 = 'xgboost_'+'paras_'+var

    ###create classifier
    clf = XGBClassifier(learning_rate=0.001, n_estimators=700, max_depth=7, subsample=0.7, scale_pos_weight=0.66)
