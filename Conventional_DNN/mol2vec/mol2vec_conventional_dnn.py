...
def fit_model(X_train, y_train, X_validation, y_validation, n, model_path, model, batch_size):
    ...
    ###fit model
    model.fit(X_train, y_train, validation_data=(X_validation, y_validation), epochs=n, batch_size=batch_size, class_weight=class_weights, callbacks=[checkpoint, es])#, callbacks=[checkpoint, es]
    ###load the best model
...
###read data
data = pd.read_csv('/account/tli/CDER/data/fps_extra/dili_1009_mol2vec.csv',low_memory=False)

X, y = data[data.Usage != 'test'].iloc[:, 7:], data[data.Usage != 'test']['DILI_label']
...
###create directory
base_path = '/account/tli/CDER/results/check/dnn/mol2vec_conventioanl_dnn'
...
###parameters
col_name2 = 'mol2vec_conventional_dnn'
...
###load the best_model
best_model = load_model('/account/tli/CDER/results/check/dnn/mol2vec_conventioanl_dnn_1/weights/mol2vec_conventional_dnn_weights.h5')
