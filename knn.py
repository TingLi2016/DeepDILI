...
paras = [l for l in itertools.product(n_neighbors, years)]
print(len(paras))
para = paras[2]
print(para)
...
X_org,  y_org = data[data.final_year<para[1]].iloc[:,2:], data[data.final_year<para[1]]['DILI_label']
X, X_val, y, y_val = train_test_split(X_org,  y_org, test_size=0.2, stratify=y_org, random_state=7)

X_test, y_test = data[data.final_year>=para[1]].iloc[:,2:], data[data.final_year>=para[1]]['DILI_label']
print(X_org.shape)
print(X.shape)
print(X_test.shape)


base_path = '/account/tli/CDER/results/sequence_feature_selection/knn/knn' + var
...
for i in range(20):
    ...
    for train_index, validation_index in skf.split(X, y):
        ...
        ### define column name
        col_name = 'knn_'+'seed_'+str(i)+'_skf_'+str(j)+'_paras_'+var+'_K_'+str(para[0])
        col_name2 = 'knn_'+'paras_'+var+'_K_'+str(para[0])
        
        ###create classifier
        clf = KNeighborsClassifier(n_neighbors=para[0])
