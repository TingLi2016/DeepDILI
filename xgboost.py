...
paras = [l for l in itertools.product(lr, n_estimators, max_depth, subsample, years)]
print(len(paras))
para = paras[105]
print(para)
...
tmp = tmp.drop([*features[:int(var)]], axis=1)
...
X_org,  y_org = data[data.final_year<para[4]].iloc[:,2:], data[data.final_year<para[4]]['DILI_label']
X, X_val, y, y_val = train_test_split(X_org,  y_org, test_size=0.2, stratify=y_org, random_state=7)

X_test, y_test = data[data.final_year>=para[4]].iloc[:,2:], data[data.final_year>=para[4]]['DILI_label']
print(X_org.shape)
print(X.shape)
print(X_test.shape)

base_path = '/account/tli/CDER/results/sequence_feature_selection/xgboost/xgboost' + var
...
for i in range(20):
    ...
    for train_index, validation_index in skf.split(X, y):
        ...
        lr, n_estimators, max_depth, subsample, years,  scale_pos_weight
        ### define column name
        col_name = 'xgboost_'+'seed_'+str(i)+'_skf_'+str(j)+'_paras_'+var+'_lr_'+str(para[0])+'_n_'+str(para[1])+'_depth_'+str(para[2])+'_subsample_'+str(para[3])
        col_name2 = 'xgboost_'+'paras_'+var
        
        ###create classifier
        #class_weight = {0:int(var1),1:(100-int(var1))}
        clf = XGBClassifier(learning_rate=para[0], n_estimators=para[1], max_depth=para[2], subsample=para[3])
