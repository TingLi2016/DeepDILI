...
paras = [l for l in itertools.product(n_estimators, max_depth, min_samples_leaf, years)]
print(len(paras))
para = paras[61]
print(para)
...
X_org,  y_org = data[data.final_year<para[3]].iloc[:,2:], data[data.final_year<para[3]]['DILI_label']
X, X_val, y, y_val = train_test_split(X_org,  y_org, test_size=0.2, stratify=y_org, random_state=7)

X_test, y_test = data[data.final_year>=para[3]].iloc[:,2:], data[data.final_year>=para[3]]['DILI_label']
print(X_org.shape)
print(X.shape)
print(X_test.shape)


base_path = '/account/tli/CDER/results/sequence_feature_selection/rf/rf' + var
...
for i in range(20):
    ...
    for train_index, validation_index in skf.split(X, y):
        ...
        ### define column name 
        col_name = 'rf_'+'seed_'+str(i)+'_skf_'+str(j)+'_paras_'+var+'_estimator_'+str(para[0])+'_depth_'+str(para[1])+'_leaf_'+str(para[2])+'_year_'+str(para[3])
        col_name2 = 'rf_'+'_paras_'+var


        ### create and fit model
        clf = RandomForestClassifier(random_state=1, n_estimators=para[0], max_depth=para[1],  min_samples_leaf=para[2], bootstrap = True, max_features='log2')
