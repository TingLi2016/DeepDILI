...
def create_model(n_dim, node, activation, optimizer):
    ...
    # The Input Layer :
    NN_model.add(Dense(10, input_dim = n_dim, kernel_initializer=he_normal, activation='tanh'))
    ...
    # Compile model
    NN_model.model.compile(loss="binary_crossentropy", optimizer=optimizers.Adam(lr=0.0001), metrics=['accuracy'])
    return NN_model

###fit model
def fit_model(X_train, y_train, X_validation, y_validation, n, model_path, model, batch_size):
    ###fit model
    model.fit(X_train, y_train, validation_data=(X_validation, y_validation), epochs=20, batch_size=8)
...
def dim_reduce(df, test_df, col_name1):
    ...
    ###parameters
    model_path = '/account/tli/CDER/script/train_validation_test/sequence_feature_selection/best_model.h5'
...
###data
path='/account/tli/CDER/results/sequence_feature_selection/data'
...
base_path='/account/tli/CDER/results/sequence_feature_selection/result'
